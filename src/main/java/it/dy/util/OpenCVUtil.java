...
//        String opencvDllName = "D:\\Downloads\\Programs\\opencv\\build\\java\\x64\\opencv_java460.dll";               //Linux 环境
        String opencvDllName = "/usr/local/share/java/opencv4/libopencv_java460.so";               //Linux中so文件地址
    ...
    public static void main(String[] args) {
        ...
        BufferedImage buff = ConverUtil.bufferRead("D:\\Downloads\\73930823f6083e79408c9845f472ee0c.jpeg");
        //全景图切割
     ...
     * panoImgPath  本地存储地址
     */
    public Mat[] shear(BufferedImage buff, int targetWidth, int targetHeight,String path) {
        Mat mat = ConverUtil.bufferToMat(buff, buff.getType());
        Mat[] cube = new Mat[6];
        for (int i = 0; i < 6; i++) {
            cube[i] = sideCubeMapImage(mat, i, targetWidth, targetHeight,path);
        }
        return cube;
    }
     ...
     * @Date:2021/10/22 13:51
     */
    private Mat sideCubeMapImage(Mat source, final int sideId, final int sideWidth, final int sideHeight,String path) {
        Mat result = new Mat(sideWidth, sideHeight, source.type());
        System.out.println("==========handle " + sideId + " start ===========");
        // 获取图片的行列数量
        float sourceWidth = source.cols();
        ...
        Imgproc.blur(source, image, new Size(3,3),new Point(-1,-1));
        if (sideId == 0) {
            ConverUtil.matSave(path+"/a.jpg", result);
        } else if (sideId == 1) {
            ConverUtil.matSave(path+"/b.jpg", result);
        } else if (sideId == 2) {
            ConverUtil.matSave(path+"/c.jpg", result);
        } else if (sideId == 3) {
            ConverUtil.matSave(path+"/d.jpg", result);
        } else if (sideId == 4) {
            //旋转角度
            result = ConverUtil.rotateRight(result, 1);
            ConverUtil.matSave(path+"/e.jpg", result);
        } else if (sideId == 5) {
            //旋转角度
            result = ConverUtil.rotateRight(result, 0);
            ConverUtil.matSave(path+"/f.jpg", result);
        }
        System.out.println("==========handle " + sideId + " over ===========");
        return result;
    }
    ...
    public Mat mergeImage(Mat[] cube, int width,String path) {
        Mat mat = new Mat(width * cube.length, width, cube[0].type());
        for (int i = 0; i < cube.length; i++) {
            Mat side = ConverUtil.matResize(cube[i], 512, 512);
            mat.put(i * 512, 0, getByte(side));
        }
        ConverUtil.matSave(path+"/preview.jpg", mat);
        return mat;
    }
